...
| **Nix Flake** | **フルコミットハッシュ (immutable)** | **意図しない更新を防止、改ざんは検知可能** |
...
- Nix Flakeはフルコミットハッシュを使いすべての依存を管理する
  - 再現性の高い開発環境を提供できる。
  - tagを侵害されても(コミットされた時点でマリシャスなコードでなければ)影響をうけない。

**学習コストは高そうに見えるがAIに書いてもらえば問題ない!!素晴らしい時代!**
